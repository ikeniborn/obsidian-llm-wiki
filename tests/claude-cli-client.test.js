...
function makeMockProcess(lines) {
...
describe("ClaudeCliClient", () => {
    ...
    it("yields text chunks from assistant_text stream-json lines", async () => {
        ...
        spawn.mockReturnValue(makeMockProcess(lines));
        const client = new ClaudeCliClient(cfg);
        const stream = await client.chat.completions.create({ model: "sonnet", messages: [{ role: "user", content: "hi" }], stream: true }, { signal: new AbortController().signal });
        const chunks = [];
    ...
    it("non-streaming returns ChatCompletion with accumulated text", async () => {
        ...
        spawn.mockReturnValue(makeMockProcess(lines));
        const client = new ClaudeCliClient(cfg);
        const resp = await client.chat.completions.create({ model: "sonnet", messages: [{ role: "user", content: "hi" }], stream: false });
        expect(resp.choices[0].message.content).toBe("world");
    });
    ...
    it("aborts subprocess on signal", async () => {
        ...
        ctrl.abort();
        const client = new ClaudeCliClient(cfg);
        await client.chat.completions.create({ model: "sonnet", messages: [{ role: "user", content: "hi" }], stream: false }, { signal: ctrl.signal });
    ...
    it("aborts non-streaming call mid-flight via signal", async () => {
        ...
        const createPromise = client.chat.completions.create({ model: "sonnet", messages: [{ role: "user", content: "hi" }], stream: false }, { signal: ctrl.signal });

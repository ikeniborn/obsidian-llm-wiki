...
describe("runQuery", () => {
    it("yields error when question is empty", async () => {
        ...
        const events = await collect(runQuery([], false, vt, makeLlm("answer"), "model", [domain], "/vault", new AbortController().signal));
    ...
    it("yields result with LLM answer", async () => {
        ...
        const events = await collect(runQuery(["What is the answer?"], false, vt, makeLlm("The answer is 42."), "model", [domain], "/vault", new AbortController().signal));
    ...
    it("saves answer page when save=true", async () => {
        ...
        const vt = new VaultTools(adapter, "/vault");
        await collect(runQuery(["What is X?"], true, vt, makeLlm("X is Y."), "model", [domain], "/vault", new AbortController().signal));
        expect(adapter.write).toHaveBeenCalled();
        const [savedPath] = adapter.write.mock.calls[0];

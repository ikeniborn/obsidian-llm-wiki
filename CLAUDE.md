...
Установка в Obsidian — симлинк папки плагина:
```bash
...
- **history**: хранится в настройках Obsidian, лимит `historyLimit` (default 20).
- **Домены**: тип `WikiDomain` в `src/types.ts` строка 8 — `"ии" | "ростелеком" | "базы-данных"`. При добавлении нового домена расширить union-тип там же.

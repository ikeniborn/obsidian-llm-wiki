import { ItemView, Modal, MarkdownRenderer, Component, Notice } from "obsidian";
...
export class LlmWikiView extends ItemView {
    ...
    getViewType() { return LLM_WIKI_VIEW_TYPE; }
    getDisplayText() { return "LLM Wiki"; }
    ...
    onOpen() {
        ...
        this.initBtn.addEventListener("click", () => {
            ...
            if (!d) {
                new Notice(i18n().view.selectDomainForInit);
                return;
            }
        ...
        this.ingestBtn.addEventListener("click", () => {
            ...
            if (!file) {
                new Notice(i18n().view.noActiveFile);
                return;
            }
    ...
    openAddDomain() {
        ...
        if (!cwd) {
            new Notice(i18n().view.cwdNotSet);
            return;
        }
    ...
    submitQuery(save) {
        ...
        if (!q) {
            new Notice(i18n().view.enterQuestion);
            return;
        }
        if (this.state === "running") {
            new Notice(i18n().view.operationInProgress);
            return;
        }

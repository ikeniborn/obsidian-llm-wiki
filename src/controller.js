...
import { validateDomainId } from "./domain-map";
...
export class WikiController {
    ...
    async ingestActive(domainId) {
        ...
        if (!file) {
            new Notice(i18n().ctrl.noActiveFile);
            return;
        }
    ...
    registerDomain(input) {
        ...
        if (err) {
            new Notice(i18n().ctrl.domainAddFailed(err));
        ...
        try {
            for await (const ev of runGen) {
                ...
                if (ev.kind === "domain_updated") {
                    ...
                    if (domain) {
                        if (ev.patch.entity_types !== undefined)
                            domain.entity_types = ev.patch.entity_types;
                        if (ev.patch.language_notes !== undefined)
